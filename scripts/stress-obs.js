...
const { TextFileBroadcastOutput } = require("../src/infrastructure/broadcast/text-file-output");
...
function snapshot(i) {
  return {
    ...
    match: {
      status: i % 2 ? "RUNNING" : "PAUSED",
      time: `00.${String(i % 60).padStart(2, "0")}`,
      timeElapsed: i,
      remainingSeconds: Math.max(180 - i, 0),
      matchDuration: 180,
    },
