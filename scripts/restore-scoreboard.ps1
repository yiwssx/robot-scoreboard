...
try {
  ...
  $listening = [bool](netstat -ano | Select-String -Pattern ':3000\s+.*LISTENING')
}
...
foreach ($name in @("data", "obs", "config")) {
  if (-not (Test-Path (Join-Path $BackupPath $name))) {
    throw "Invalid backup: missing $name directory"
  }
}
