...
const { createFieldReadiness, REQUIRED_PUBLIC_ASSETS } = require("../server/diagnostics/field-readiness");
...
test("field readiness validates runtime paths, built client, broadcast and required pages", async () => {
...
test("field readiness fails when a compiled client entry is missing", async () => {
  const item = await fixture();
  try {
