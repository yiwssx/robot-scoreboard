...
const { Persistence } = require("../../storage/persistence/file-store");
const { TextFileBroadcastOutput } = require("../../broadcast/outputs/text-file-output");
const { createBroadcastService } = require("../../broadcast/broadcast-service");
const { EventLog } = require("../../storage/logging/event-log");
const { createScoreboardState, STATUSES } = require("../runtime/scoreboard-state");
const { createScoreboardRuntime } = require("../runtime/scoreboard-runtime");
...
function createScoreboard({ dataDir, obsDir, onUpdate = () => {}, rules: suppliedRules = {}, obsControl = null }) {
  ...
  return {
    initialize, shutdown, forcePersist,
